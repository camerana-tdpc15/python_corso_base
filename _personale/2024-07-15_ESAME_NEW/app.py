...
def locali():
    if 'user_id' not in session:
        ...
        pass
    return render_template('locali.html')
...
def get_locali():
    ...
    if order == 'asc':
        locali = Locale.query.order_by(Locale.nome_locale).all()
    elif order == 'desc':
        locali = Locale.query.order_by(Locale.nome_locale.desc()).all()
    else:
    ...
    for locale in locali:
        ...
        locali_data.append(dict_locale)

    return jsonify(locali_data)

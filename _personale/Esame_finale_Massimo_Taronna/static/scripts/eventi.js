    ...
    fetch('/api/eventi_con_repliche')
        ...
        .then(eventi => {
            ...
            eventi.forEach(evento => {
                ...
                col.innerHTML = `
                    <div class="card">
                        <img src="/static/imgs/${evento.immagine}" class="card-img-top" alt="${evento.nome}">
                        <div class="card-body">
                            <h3 class="card-title text-center"><b>${evento.nome_evento}</b></h3>
                            <hr>
                            <h4>Dove: ${evento.luogo}</h4>
                            <hr>
                            <ul class="list-group list-group-flush">
                            <h4>Quando:</h4>
                                ${evento.repliche.map(replica => `
                             ...
                             ${evento.repliche.some(replica => !replica.annullato) ? `
                                <button class="btn btn-primary mt-3" onclick="apriModalPrenotazione(${evento.id}, ${JSON.stringify(evento.repliche.filter(replica => !replica.annullato)).replace(/"/g, '&quot;')})">Prenota</button>
                            ` : ''}
                        </div>
                    </div>
                `;
...
function apriModalPrenotazione(eventoId, repliche) {
    ...
    replicaSelect.innerHTML = repliche.map(replica => `
...
function prenotaReplica(replicaId, quantita, modal) {
    fetch(`/prenota/${replicaId}`, {
        ...
        if (response.ok) {
            ...
            response.json().then(data => {
                alert(data.error);
                if (data.error.includes('Hai già una prenotazione per questa replica')) {
                    modal.hide();
                    window.location.href = '/prenotazioni';
                }
            });

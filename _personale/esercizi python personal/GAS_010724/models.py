...
class User(db.Model):
    ...
    nome = db.Column(db.Column(50), nullable = False)
    cognome = db.Column(db.Column(50), nullable = False)
...
class Produttore(db.Model):
    ...
    telefono = db.Column(db.Integer(20), nullable = False)
    email =db.Column(db.String(50), nullable = False)

class Prodotto(db.Model):
    __tablemane__='prodotti'
    id = db.Column(db.Integer, prymary_key=True, autoincrement = True)
...
class Lotto(db.Model):
    ...
    qta_unita_misura = db.Column(db.Strind(10), nullable = False)
...
def init_db(app):   
        #Crea le tabele se non ci sono gia
        db.create_all()